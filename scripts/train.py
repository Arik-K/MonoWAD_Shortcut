...
def main(config="config/config.py", experiment_name="default", world_size=1, local_rank=-1):
    ...
    set_random_seed(cfg.trainer.random_seed if cfg.trainer.random_seed is not None else 444)
    ...
    if is_logging:
        ...
        if os.path.isdir(recorder_dir):
            ...
            print("Cleaned up the recorder directory of {}".format(recorder_dir))
    ...
    if is_distributed:
        ...
        detector = torch.nn.parallel.DistributedDataParallel(detector.cuda(), device_ids=[gpu], output_device=gpu, find_unused_parameters=True)
    else:
    ...
    for epoch_num in range(cfg.trainer.max_epochs):
        ...
        if is_logging:
            torch.save(detector.module.state_dict() if is_distributed else detector.state_dict(), os.path.join(
                cfg.path.checkpoint_path, '{}_latest.pth'.format(cfg.detector.name)
            ))
        if is_logging and (epoch_num + 1) % cfg.trainer.save_iter == 0:
            torch.save(detector.module.state_dict() if is_distributed else detector.state_dict(), os.path.join(
                cfg.path.checkpoint_path, '{}_{}.pth'.format(cfg.detector.name, epoch_num)
            ))
        ...
        if is_distributed:
            torch.distributed.barrier()  # wait until all finish an epoch
