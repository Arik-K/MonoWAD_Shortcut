...
class MonoWAD(nn.Module):
    def __init__(self, backbone_arguments=dict()):
        ...
        pretrained_path = 'workdirs/MonoWAD/checkpoint/MonoWAD_3D.pth' # change based on path you use
    ...
    def forward(self, x):
        ...
        # 1. Parsing Logic: Handle both List and Dict inputs safely
        if isinstance(x, list):
            ...
            # Dictionary mode (what Detector.py uses)
            images = x['image']
            training = x.get('training', False)
            if training:
                foggy_images = x['foggy']
        else:
            # Fallback
            images = x
            training = False
        ...
        if training:
            ...
            # --- PACK ALL LOSSES HERE ---
            l_proposed = diffusion_losses['loss_total'] + l_ckr
            
            # Helper to create a zero tensor on the correct device
            zero_tensor = torch.tensor(0.0, device=images.device)

            loss_dict_details = {
                'l_grounding': diffusion_losses.get('l_grounding', zero_tensor),
                'l_consistency': diffusion_losses.get('l_consistency', zero_tensor),
                'l_codebook': l_ckr
            }
        ...
        img_feat = x_enhanced + self.img_conv(x_enhanced.clone())
        img_feat = img_feat.view(N, H*W, C)
        feat = self.dtr(depth_feat, img_feat, depth_emb)
        feat = feat.permute(0, 2, 1).view(N,C,H,W)
        
        if training:
            return feat, depth, l_proposed, loss_dict_details
        else:
            return feat, depth
