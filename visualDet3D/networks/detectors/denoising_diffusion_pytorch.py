...
class ShortcutDiffusion(nn.Module):
    ...
    def forward(self, img_clean, img_foggy, codebook=None):
        ...
        with torch.no_grad():
            ...
            # Consistency Target (s_target = (s1 + s2) / 2)
            if mask_shortcut.any():
        ...
        loss_grounding = F.mse_loss(pred_student[~mask_shortcut], target[~mask_shortcut])
        
        loss_consistency = torch.tensor(0., device=device)
        if mask_shortcut.any():
            loss_consistency = F.mse_loss(pred_student[mask_shortcut], target[mask_shortcut])

        # Return a dictionary instead of a single tensor
        return {
